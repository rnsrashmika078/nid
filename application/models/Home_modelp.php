...
class Home_modelp extends CI_Model {
 ...
 // if($postData['search'] ){
  ...
  /**
    ...
    {
       $this->db->select('BaseTbl.instrument_id,Institute.institute_id,Institute.name,BaseTbl.instrument_id,laboratory.laboratory_name,laboratory.laboratory_address,laboratory.accreditation_type,laboratory.latitude,laboratory.longitude,iType.instrument_type,iType.keywords,BaseTbl.contact_person_name,BaseTbl.contact_person_email,BaseTbl.contact_person_phone_number,BaseTbl.contact_person_mobile_number');
        $this->db->from('instrument as BaseTbl');
        ...
        if(!empty($searchText)) {
            $likeCriteria =  "(
			            ...
			            MATCH (iType.keywords) AGAINST ('%".$searchText."%' IN NATURAL LANGUAGE MODE)	 
						OR MATCH (iType.instrument_type) AGAINST ('%".$searchText."%' IN NATURAL LANGUAGE MODE) 	 
			           ...
			           OR  Institute.name  LIKE '%".$searchText."%'
			            OR iType.instrument_type  LIKE '%".$searchText."%'
						   ...
						   OR soundex(iType.keywords)  LIKE soundex('%".$searchText."%')
						    OR soundex(iType.instrument_type)  LIKE soundex('%".$searchText."%')
  ...
  /**
    ...
    function instrumentListing($searchText = '', $page=0, $segment=0)
    {
       // $this->db->select('BaseTbl.instrument_id,BaseTbl.instrument_name,BaseTbl.inst_description,BaseTbl.model,Institute.name,Institute.address,iType.instrument_type_id,iType.instrument_type,iType.parameters,iType.keywords,BaseTbl.inst_keywords,BaseTbl.contact_person_name,BaseTbl.contact_person_email,BaseTbl.contact_person_phone_number,BaseTbl.contact_person_mobile_number,BaseTbl.p_categories,BaseTbl.image_upload1,BaseTbl.image_upload2,BaseTbl.image_upload3,BaseTbl.image_upload4,BaseTbl.record_status');
        ...
        $this->db->distinct();
		$this->db->select('BaseTbl.instrument_id,Institute.institute_id,Institute.name,laboratory.laboratory_name,laboratory.laboratory_address,laboratory.accreditation_type,laboratory.latitude,laboratory.longitude,iType.instrument_type,iType.keywords,BaseTbl.contact_person_name,BaseTbl.contact_person_email,BaseTbl.contact_person_phone_number,BaseTbl.contact_person_mobile_number');
		$this->db->from('instrument as BaseTbl');
        $this->db->join('users as U','U.id = BaseTbl.created_user_id');
        $this->db->join('user_types as Roles','Roles.user_type_id = U.user_type_id');
        $this->db->join('institutes as Institute','Institute.institute_id = BaseTbl.institute_id');
